...
import { fallbackTrafficChannels } from "@/lib/services/growth-agent-defaults";
...
export async function runGrowthAgentManualSync(storeId?: string) {
  ...
  const [dailyMetrics, orders, products, connections, attributionSignals] = await Promise.all([
  ...
  const trafficByChannel = Object.keys(orderSourceCounts).length
    ...
    : fallbackTrafficChannels;

  const topProducts = products.slice(0, 5).map((product) => ({ productId: product.id, title: product.title, estimatedInventory: Math.max(4, Math.round((product.price / Math.max(product.estimatedCost, 1)) * 3)), collection: product.collection }));
